\chapter{Fault detection in Utility Scale Photovoltaic Plants}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth, trim={1cm 0cm 5cm 1cm},
...
\begin{itemize}
    \item Shading: partial coverage, temporary or not, of a PV array or module.
    Might result in Hot Spot fault.
    \item Soiling: dirt accumulation, blocking sunlight from reaching PV Cells.
    Might also result in Hot Spot fault.
    \item Short circuit: either line-line or line-ground.
    \item Open circuit: connection breakage between modules.
    \item DC arc fault: eletricity plasma arc formed on broken connections.
\end{itemize}

Due to the difficulty of classifying some of these faults, given their similarity on the consequent effect in the system, it will be seen in further sections that most fault detection algorithms only endeavor to classify between two to five types of faults.
