...
% TODO em vez de tabela e gráfico, colocar só gráfico com anotações dos valores (e.g. aos 10s)
\begin{figure}[h!]
    \begin{floatrow}
    \capbtabbox{%
    \resizebox{4cm}{!}{%
...
% nao esquecer de adicionar referências às ferramentas mencionadas
...
% adicionar ref para o YAML

Configuring cells can be done through a simple configuration file using the YAML protocol (one per cell). It should contain the definition of its variables, database credentials, hub credentials, and all other parameters. % meter anexo com screenshot de configuração exemplo
