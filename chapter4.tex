...
\paragraph{Test nº2} In this next experiment, we observe the impact of disagreeing terms $b$ and $c$.
...
In figure \ref{fig:trust_test_2_a}, it is clear that, for all methods, an increase in $b$ leads to lower trust. All except the new algorithm tend towards similar asymptotes, near 0.3. However, the expectation is that an increase in disagreement should lead to zero trust, precisely what happens in the proposed method.
...
With figure \ref{fig:trust_test_2_b}, we can observe the asymmetry of Theil's U coefficient, opposing to what figure \ref{fig:trust_test_2_a} showed. It does not represent the trust between cell nº1 and cell nº2, but the trust of nº1 in nº2. The rest of the methods are symmetrical, thus presenting the same results.

\paragraph{Test nº3} This last experiment 
...
Intending to attribute good use to all the processes developed for the cell, we introduce a state system that seeks to provide short and critical information about it.
