...
After n the potential for time similarity extraction in the intrinsic inference process of the cell, we encounter another application for the "activations". Given that it represents the past instances of the cell that resemble the current inputs, it allows comparing different cell's behavior. This is the peculiar characteristic of the CellTAN that allows it to be used without data privacy issues and even normalization.

Time is inherently normalized, considering all commponents experience it equally. which allows the network to compare systems without caring about their actual variables. This is what allows the proposed tool to be generalized beyond PV applications.
...
Figure \ref{fig:trust_test_1_b} extends the previous analysis, showcasing that all methods have asymptotical behavior, settling in a value greater than the start. All except Theil's U have asymptotes deemed acceptable for this test: since disagreement terms $b$ and $c$ are non-null and relatively close to $a$, we expect that the trust index doesn't reach a value near the max, remaining closer to a measure of "half trust".

\paragraph{Experiment nº2} Now, we observe the impact of the disagreeing terms $b$ and $c$ in the same coefficients, by varying one while maintaining all other fixed. The expectation is that an increase in disagreement should lead to zero trust.
